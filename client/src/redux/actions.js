...
export const getVideogames = () => {
    return async function (dispatch) {
        let videogames = await axios.get('/videogame');
...
export const searchByName = (name) => {
    return async function (dispatch) {
      try {
        let videogamesSearch = axios.get(`/videogame?name=${name}`)
...
export const getVideogamesDetail = (id) => {
    return async function (dispatch) {
        try {
            let videogamesDetail = await axios.get(`/videogame/${id}`)
...
export const getGenero = () => {
    return async function (dispatch) {
      ...
      var genero = await axios.get("/genero");
      return dispatch({
        type: "GET_GENERO",
        payload: genero.data,
      });
